...
const skipFoldersPattern = new RegExp(
	'styles|icon|lib|__tests__|__test__|images|__snapshots__|node_modules|dist|.git|types|assets'
);

// const includeFoldersPattern = new RegExp('packages/');
const regexPattern = new RegExp('.*.js|.*.ts|.*.tsx');
const excludePattern = new RegExp(
	'lib|package.json|.*.md|messages.*|.*.css|.*.scss|.*.svg|yarn.lock|package-lock.json|.*.d.ts|.*.test.ts'
);
